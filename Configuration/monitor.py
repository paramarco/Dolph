...
with open('/home/dolph_user/data/5/Dolph/log/Dolph.log') as f:
    lines = f.readlines()
...
for line in lines:
    ...
    if 'Order is Filled-Monitored' in line:
        entries += 1
        # Find the security from the triggerExitOrder line that follows (same order)
    ...
    if 'exitOrder' in line and 'Filled' in line and 'deleted' in line:
        tp += 1
        # Get the order id to match with security
        oid = re.search(r'exitOrder: (\d+)', line)
        last_tp_sec = ''
        if oid:
            oid_val = oid.group(1)
            # Search backwards for this order's security
            for prev in lines:
                if oid_val in prev and 'triggerExitOrder' in prev:
                    ...
                    if sm:
                        ...
                        break
    if 'STP' in line and 'status=Filled' in line: sl += 1
