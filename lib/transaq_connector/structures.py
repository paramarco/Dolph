...
def parse(xml):
    ...
    logging.error(u"Неподдерживаемый xml, не распарсился нихрена! Типа %s" % xml[:10])
    logging.debug(xml)
    return None
    ...
    def __repr__(self):
        ...
        for m in inspect.getmembers(cls):
            ...
            if isinstance(val, Field):
                listNameVal.append( m )
        
        for (name, val) in listNameVal:
...
class Union(Entity):
    """
    Идентификатор Союза пользователей.
    """
    ROOT_NAME = 'union'
    id = StringField('@id')
    active = SimpleBooleanField('@remove', 'false', 'true')
